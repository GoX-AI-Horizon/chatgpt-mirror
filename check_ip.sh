...
# 检测 ChatGPT 可用性
chatgpt_status=$(curl -s -o /dev/null -w "%{http_code}" -L --socks5-hostname 127.0.0.1:1080 https://chatgpt.com)
...
# 检测 Netflix 可用性
netflix_status=$(curl -s -o /dev/null -w "%{http_code}" -L --socks5-hostname 127.0.0.1:1080 https://www.netflix.com)
